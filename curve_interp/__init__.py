...
def get_nearest(x,xarr,x_id,y_curve,n=3,ignore_same=False):
    """
    ...
    x_id = x_id[not_nan]
    dist = np.abs(xarr-x)
    if ignore_same == True and dist[0] < 1e-12:
        dist = dist[1:]
        x_id = x_id[1:]
...
def curve_interp(x_array, x_curve, y_curve, n=3, degree=2, extrap_deg=1, extrap_n=2, iterate=0, ignore_same=False):
    ...
    ignore_same: bool (default=False)
        ...
        ignore x value from x_curve in fit
    ...
    for i in range(len(x_array)):
        ...
        if i != 0:
            ...
            nn_id_new = get_nearest(x,x_curve,x_id,y_curve,n=n_fit,ignore_same=ignore_same)
